<template>
  <div class="base-table">
    <a-row
      ...
      class="pd-b-10"
    >
      <a-row class="mg-b-10" v-if="visibleFormItems.length">
        <a-form-model
          layout="vertical"
          :colon="false"
          :model="params"
    ...
    >
      <template
        ...
        <slot :name="column.field" :row="rawTableData[rowIndex]">{{ row[column.field] }}</slot>
      </template>
...
export default {
  components: { ExportButton, ASRangePicker: SRangePicker, SUpload, ToggleList },
  props: {
    isSelect: {
      type: Boolean,
      default: false
    ...
    exportApi: {
      type: String,
      default: 'exportEntityData'
    },
    exportFileApi: {
      type: String,
      default: 'getEntityExportTask'
    },
    extraData: {
      type: Object,
      default () {
  ...
  computed: {
    ...
    completeColumns () {
      ...
      if (this.serial && !columns.some(x => x.type === 'seq')) {
        columns.unshift({
          title: 'No.',
          type: 'seq',
          field: 'serial',
  ...
  methods: {
    handleToggleChange (list) {
      this.visibleFormItems = list
    },
    ...
    initParams () {
      ...
      this.formItems.forEach(item => {
        if (item.default) {
          this.$set(this.params, item.key, item.default)
        } else if (['rangeInput'].includes(item.type)) {
    ...
    async loadTableData () {
      ...
      const result = await this.$api[this.queryApiName](...this.extraArgs, requestData).finally(() => (this.queryLoading = false))
      if (this.hidePagination) {
      ...
      if (!this.hidePagination) {
        this.pagination.total = result.total_elements || result.total
