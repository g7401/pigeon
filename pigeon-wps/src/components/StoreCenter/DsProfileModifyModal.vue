    ...
    >
      <template>
        ...
        <a-card size="small" v-if="processorUid">
          <Processor ref="processor" :uid="processorUid" :dataProps="item.processor_data_props" @changeData="handleDataChange"></Processor>
        </a-card>
      </template>
    </a-modal>
  </div>
...
export default {
  ...
  computed: {
    title () {
      return 'Modify Data Source Connection Profile'
    }
  },
  async created () {
    console.log('this.item', this.item)
    this.formData.name = this.item.connection_profile_name
    this.formData.uid = this.item.ds_connection_profile_uid
  ...
  methods: {
    ...
    save () {
      return this.$api.updateDsProfile(this.formData.uid, this.formData.name, this.formData.description, this.formData.ds_type_uid, this.formData.processor_data_props)
    },
    async handleOk () {
