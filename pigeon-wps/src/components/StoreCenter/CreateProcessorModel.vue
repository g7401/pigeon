...
export default {
  ...
  methods: {
    async afterUpload (fileId) {
      ...
      const resp = await this.$api.parseProcessor(this.type.toUpperCase(), fileId)
      if (resp && resp.success && resp.object) {
        this.uploadedProcessor = resp.object
      }
    },
    async getCreateProcessor () {
      const resp = await this.$api.createProcessor(this.type.toUpperCase(), this.programPackageFileId)
