    ...
    >
      ...
      >
        ...
        >
          ...
          <template
            slot="table_col_alias"
            slot-scope="text, record"
          >
            <a-input
              v-model="record.tableColumnAlias"
              @change="change"
            />
          </template>
          <template
...
export default {
  ...
  data () {
    return {
      ...
      parseApi: '/kapok-dis/master/templates/parse_ds_template_from_excel',
      columns: [
        {
          title: 'No.',
          key: 'serial',
        ...
        },
        {
          ...
          scopedSlots: { customRender: 'table_col_alias' }
        },
        {
  ...
  methods: {
    ...
    initData () {
      ...
      if (
        ...
        !this.dataProperties.sourceDataFields ||
        !this.dataProperties.sourceDataFields.length
      ) {
        return
      }
      this.dataProperties.tables.forEach(table => {
        table.fields &&
          table.fields.forEach((item, index) => {
            Object.assign(item, this.dataProperties.sourceDataFields[index])
          })
      })
    },
