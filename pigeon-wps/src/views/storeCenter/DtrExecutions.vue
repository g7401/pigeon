  ...
  >
    ...
    <div class="output-table">
        ...
        >
          <a-col :span="20">
            ...
            >
              <a-row :gutter="24">
                <a-col :span="6">
                  <a-form-model-item label="Data Target">
                    <DtTreeSelect
                      v-model="params.dt_uid"
                      @input="handleSearch"
                    ></DtTreeSelect>
                  </a-form-model-item>
                </a-col>
      ...
      >
        ...
        >Create DTR Process Instance</a-button>
      </a-row> -->
      <a-table
        :bordered="true"
        class="selectable-table more-small-table"
        rowKey="dtr_process_inst_uid"
        v-if="columns.length > 1"
        :scroll="scroll"
        :columns="columns"
      ...
      >
        ...
        >
          <a-button-group>
            ...
            >
              <a-button
                v-if="button.showStatus.includes(record.dtr_process_instance_status)"
    ...
    >
      <h3 class="title">Below is the DTR Process Pipeline of selected DTR Process Instance:({{ activeInstanceId }})</h3>
      <a-tabs
        type="card"
        size="large"
        v-model="activeTab"
      ...
      >
        <a-tab-pane
          v-for="tab in tabs"
          :key="tab.key"
          :tab="firstLetterUpper(tab.tab)"
        >
          <template v-if="taskInstance">
            <TaskInstanceInfo
              type="transform"
              :taskInstance="taskInstance"
              apiName="getDtrOutTableData"
            ></TaskInstanceInfo>
          </template>
          <template v-else>
...
export default {
  ...
  data () {
    return {
      ...
      rawColumns: [
        ...
        {
          title: 'DTR Process Definition ID',
          dataIndex: 'dtr_process_def_uid',
          width: 250,
          ellipsis: true
        },
        {
          title: 'DTR Process Instance ID',
          dataIndex: 'dtr_process_inst_uid',
          width: 250,
          ellipsis: true
        },
        {
          title: 'DTR Process Instance Status',
          dataIndex: 'dtr_process_inst_status',
          width: 150,
          ellipsis: true
        },
        {
  ...
  methods: {
    ...
    async loadDtrDetail (dtrDefUid) {
      const resp = await this.$api.getDtrDefDetail(dtrDefUid)
      if (!resp || !resp.object) return
      this.tabs = resp.object.list_of_dtr_task_def.map(x => ({
        tab: x.name,
        key: x.uid
      }))
      if (this.tabs.length) {
    ...
    async loadTask () {
      ...
      const apiKey = `getDtrTransformDetail`
      this.taskLoading = true
      const resp = await this.$api[apiKey](this.activeInstanceId, this.activeTab).finally(() => {
        this.taskLoading = false
      })
      if (resp.success && resp.object) {
        this.taskInstance = resp.object
        console.log('this.taskInstance', this.taskInstance)
      }
    },
    handleTabChange () {
      this.loadTask()
    ...
    async changeStatus (item, type) {
      ...
      const resp = await this.$api.deleteDtrProcessInst(item.dtr_process_inst_uid).finally(() => (this.statusLoading = false))
    ...
    reset () {
      this.params = Object.assign({}, { dt_uid: this.params.dt_uid })
      this.selectedDateRange = null
      this.tableData = []
      this.handleSearch()
    },
    async clickRow (record) {
      console.log('record', record)
      this.activeInstanceId = record.dtr_process_inst_uid
      await this.loadDtrDetail(record.dtr_process_def_uid)
      await this.loadTask()
    },
    rowClassName (record) {
      if (this.activeInstanceId && this.activeInstanceId === record.dtr_process_inst_uid) {
        return 'active-row'
      }
    },
    customRow (record) {
