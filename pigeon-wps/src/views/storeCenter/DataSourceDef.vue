  ...
  >
    ...
    <a-row class="toolbar-box">
      <a-button
        ...
        type="primary"
        icon="plus-circle"
        @click="addChild(dsTree[0])"
      >Create Data Source</a-button>
    </a-row>
    <a-table
      v-if="tableVisiable"
      rowKey="uid"
    ...
    >
      <template
        ...
        slot-scope="text"
      >
        <span v-if="text.indexOf(params.name) > -1">
...
import DataSourceModal from '@/components/StoreCenter/DataSourceModal'
...
export default {
  ...
  computed: {
    tableData () {
      return this.getFormatTree(this.dsTree && this.dsTree.length && this.dsTree[0].children || [])
    },
    filterTableData () {
  ...
  methods: {
    ...
    filterHasSearchValue (treeData, searchValue, deep = true) {
      ...
      const newTreeData = copyTree.filter(x => {
        ...
        if (x[this.replaceFields.children] && hasValue !== 1) {
          x[this.replaceFields.children] = this.filterHasSearchValue(x[this.replaceFields.children], searchValue, !hasValue)
        }
        return hasValue
      })
      return newTreeData
    ...
    async loadDataSourceList () {
      ...
      await this.$store.dispatch('getDataSourceTree', false).finally(() => {
        this.treeLoading = false
      })
    },
    async del (dataSource) {
      this.deleteLoading = true
      await this.$api.deleteDataSource(dataSource.uid).finally(() => (this.deleteLoading = false))
      this.loadDataSourceList()
    }
  }
}
