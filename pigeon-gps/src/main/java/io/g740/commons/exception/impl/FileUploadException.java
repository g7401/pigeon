package io.g740.commons.exception.impl;

import io.g740.commons.exception.BasicException;

import static io.g740.commons.exception.impl.CommonErrorCodeI18N.FILE_UPLOAD_ERROR;

/**
 * @author : zxiuwu
 * @version : V1.0
 * @function :
 * @date : 2019/10/14 13:53
 * @description :
 */
public class FileUploadException extends BasicException {
    public FileUploadException(String errMessage) {
        super(FILE_UPLOAD_ERROR, errMessage);
    }

    public FileUploadException(String errMessage, Throwable e) {
        super(FILE_UPLOAD_ERROR, errMessage, e);
    }
}
