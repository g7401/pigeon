package io.g740.components.uid.tinyid.dao.impl;
...
public class TinyIdInfoDAOImpl implements TinyIdInfoDAO {
    ...
    public TinyIdInfo queryByBizType(String bizType) {
        String sql = "select id, biz_type, begin_id, max_id," +
                ...
                " from to_tiny_id_info where biz_type = ?";
    ...
    public int updateMaxId(Long id, Long newMaxId, Long oldMaxId, Long version, String bizType) {
        String sql = "update to_tiny_id_info set max_id= ?," +
    ...
    @Override
    public void create(TinyIdInfo tinyIdInfo){
        String sql = "insert into to_tiny_id_info(biz_type, begin_id, max_id, step, delta, remainder, create_time, update_time, version) " +
                "values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, tinyIdInfo.getBizType(), tinyIdInfo.getBeginId(), tinyIdInfo.getMaxId(),
                tinyIdInfo.getStep(), tinyIdInfo.getDelta(), tinyIdInfo.getRemainder(), tinyIdInfo.getCreateTime(),
                tinyIdInfo.getUpdateTime(), tinyIdInfo.getVersion());
    }
    ...
    public static class TinyIdInfoRowMapper implements RowMapper<TinyIdInfo> {
        ...
        public TinyIdInfo mapRow(ResultSet resultSet, int i) throws SQLException {
            ...
            tinyIdInfo.setCreateTime(resultSet.getTimestamp("create_time"));
            tinyIdInfo.setUpdateTime(resultSet.getTimestamp("update_time"));
